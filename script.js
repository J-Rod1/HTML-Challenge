...
function showMeme() {
  ...
  const randomMemeURL = memes[randomNumber(memes.length)]; // get random meme URL string
  
  const newMemeElement = document.createElement("img");   // new 'img' element object created
  newMemeElement.setAttribute("src", randomMemeURL);  // set new Meme element to new URL string
  const memeDivContainer = document.querySelector('.meme-content'); // meme div container
  
  memeDivContainer.appendChild(newMemeElement); // ADD NEW IMAGE TO DOCUMENT, DISPLAYS TO USER
}
...
function showJoke() {
  clearAll();
  // Get random joke STRING
  const randomJokeString = jokes[randomNumber(jokes.length)]; 
  
  //Create Element object/tag of "p" type, add new joke string to it
  const newJokeElement = document.createElement("p"); 
  newJokeElement.textContent = randomJokeString;  
  
  //Get div container and place into variable, then append 'p' element to it 
  const jokeDivContainer = document.querySelector('.joke-content'); 
  jokeDivContainer.appendChild(newJokeElement); 
}
...
function showQuote() {
  ...
  const randomQuoteString = quotes[randomNum].quote; //Quote string
  const randomAuthorString = quotes[randomNum].author; //Author string
  
  // Quote String Element
  const newQuoteElement = document.createElement("p"); 
  newQuoteElement.textContent = randomQuoteString;  
  
  // Author String Element 
  const newAuthorElement = document.createElement("p"); 
  newAuthorElement.textContent = randomAuthorString;
  
  //Get div container and place into variable, then append 'p' element to it 
  const quoteDivContainer = document.querySelector('.quote-content'); 
  quoteDivContainer.appendChild(newQuoteElement); 
  quoteDivContainer.appendChild(newAuthorElement); 
}
...
function showRiddle() {
    clearAll();
    const randomNum = randomNumber(riddles.length);
  
  const randomRiddleQuestionString = riddles[randomNum].question; 
  const randomRiddleAnswerString = riddles[randomNum].answer;   
  
  //Riddle QUESTION P element
  const newRiddleQuestionElement = document.createElement("p"); 
  newRiddleQuestionElement.textContent = randomRiddleQuestionString;  
  
    //Riddle ANSWER P element
  const newRiddleAnswerElement = document.createElement("p"); 
  newRiddleAnswerElement.textContent = " - " + randomRiddleAnswerString; 
  newRiddleAnswerElement.setAttribute("id", "riddle-answer");
  newRiddleAnswerElement.hidden = true;
  
  //Append Riddle Question to DIV container
  const riddleDivContainer = document.querySelector('.riddle-content'); 
  riddleDivContainer.appendChild(newRiddleQuestionElement); 
  riddleDivContainer.appendChild(newRiddleAnswerElement); 
}
...
function revealAnswers() {
  ...
  if (riddle && riddleAnswer.hidden) {
    riddleAnswer.hidden = false; // Show riddle answer (no longer hidden)
  }
  else if (riddle && riddleAnswer) {
    alert("The riddle answer is already displayed.");
  }
  else {
    alert("There is no riddle to show an answer for yet. Click on 'Riddle Me' button first.");
  }
}
